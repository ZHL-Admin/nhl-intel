...
// Skater radar spokes grouped into families. Reordering by family makes the radar read as regions,
// and the contiguous runs become the arc-group labels (skating / offense / defense / usage).
...
// Clockwise from the top: skating, offense, defense, then usage/deployment.
const FAMILY_ORDER = ['Skating', 'Offense', 'Defense', 'Deployment']

/** Reorder spokes by family so the radar shape reads as regions, and emit arc-group labels over the
 *  contiguous runs. Spokes without a family mapping (e.g. goalie spokes) pass through ungrouped. */
