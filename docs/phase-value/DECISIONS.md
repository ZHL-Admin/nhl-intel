...
**PV-D018 | 2026-07-22 (Stage 4, DERIVED — §6 to be owner-confirmed) | Goals accounting from the published league constants.**
The component coefficients are rates in per-60-of-EXPOSURE units (deny: episodes per 60 min OUTSIDE exposure; suppress: xG per 60 min IN-ZONE exposure). To price them per 60 of ICE time and in goals, using `phase_league_constants`:
`deny_g60 = deny * (s_out_min_per_60/60) * c_seq_xg_nonfo`  (episodes→ice-time share→xG per non-faceoff episode),
`suppress_g60 = suppress * (s_in_min_per_60/60)`  (already xG; scale to the in-zone share of ice time),
`pv_def_g60 = deny_g60 + suppress_g60`. escape is published as a RATE in v1 (favourable ends per 60 min in-zone), NOT priced into `pv_def_g60`. `pv_def_g60_sd` is left NULL this pass: the composite sd must come from SHARED bootstrap resamples (deny and suppress are positively correlated through the same game-resampling; quadrature would understate it) — that joint bootstrap is the next construction step, deliberately not shipped as a wrong number. This formula is DERIVED from the constants' documented meanings (stage2-acceptance.md), not from a verbatim §6 spec; flagged for owner confirmation before finalize, and §6 will be written to match whatever the owner rules.
