...
`pv_def_g60 = deny_g60 + suppress_g60`. escape is published as a RATE in v1 (favourable ends per 60 min in-zone), NOT priced into `pv_def_g60`. IMPLEMENTED MATCH: `assemble_phase_value._price` reproduces §8.1 line-for-line; the only correction from the first draft was ADDING the `* cal` factor (it had been omitted; numerically nil today since `xg_calibration = 1.000000`, but now exact). `pv_def_g60_sd` is left NULL this pass: §8.1 requires the composite sd from WITHIN-RESAMPLE composites (not quadrature — deny and suppress are positively correlated through the same game-resampling); that joint bootstrap is the next construction step and is deliberately not shipped as a wrong number. NOTE for review: because g60 pricing makes `deny_g60` ≈ ¼ the magnitude of `suppress_g60`, the composite `pv_def_g60` is suppress-dominated and correlates ~0.87 with `def_impact`; the novel orthogonal channels (deny r≈0.42, escape r≈0) are real but diluted in the goal-unit sum — flagged for the owner as a weighting question for v1 (component-level publication vs the composite).
