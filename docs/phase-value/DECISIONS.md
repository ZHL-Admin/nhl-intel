...
**PV-D016 | 2026-07-22 (sprite-audit follow-up, report-only) | Blocked-shot false rush-qualifiers are attacker-owned D-zone, not N-zone — no clean owner×zone surgical fix.**
Two-way split of the blocked-shot rush qualifiers on the 487 no-tracking-entry rush-labeled goals (E1c): **408/408 are attacker-owned with `zone_code='D'`** — zero defender-owned, zero N-zone. This **refutes** the anticipated N-zone/defender-owned concentration. Mechanism: `seq_rush` keeps `zone_code` for attacker-owned events, so a block the scoring team owns coded in its own defensive zone (`'D'`) passes the D/N precursor and fires the rush label; because these goals are `established_full_window` (puck tracked in the O-zone the whole 8 s), the block's recorded D-zone location/timing conflicts with the tracked possession — the block coordinate (or its ±2 s timing) is the unreliable input, not the owner/zone rule.
v1.1 note (surgical, corrected): the false qualifiers occupy the **same owner×zone cell** as the legitimate _block-in-own-D → counterattack_ precursor, so **no clean owner×zone rule** separates them and the _drop-N-zone-blocks_ candidate does not apply (there are none). The fix a v1.1 pass would need is a **location/timing-consistency check** on the block (tracking-only at goals); **wholesale blocked-shot exclusion remains wrong** — it would erase the legitimate cell. No PV action; the state engine is frozen (PV-D014).
**Parked for the owner (platform-wide, do-not-touch here):** `int_shot_sequence.seq_rush` shares this qualification rule verbatim, so the **same mechanism exists in the production sequence engine** — it feeds xG features, team identity, and the radar Rush Offense surface. The **mechanism transfers; the measured false-qualifier rate does not** (it is a goal-conditioned subpopulation — this audit only sees goals). No PV action and the do-not-touch on the production sequence engine applies; whether to add the location/timing-consistency guard to `seq_rush` platform-wide is an **owner decision for a future platform pass**, not taken here.
